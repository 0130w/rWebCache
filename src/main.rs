...
fn main() {
    ...
    thread::spawn(|| {
        ...
        web_server.insert("owo".to_string(), "qwq".to_string());
    ...
    thread::spawn(|| {
        ...
        match stream {
            Ok(mut stream) => {
                println!("Connect successfully!");
                let contents = guest.get_device_type().to_string() + 
                "\nowo";
    ...
    thread::sleep(Duration::from_secs(3));    // wait for all threads finished

    thread::spawn(|| {
        let mut guest = Guest::init(0);
        let guest_listener = guest.bind("127.0.0.1:3004".to_string());
        let mut stream = TcpStream::connect("127.0.0.1:3002");
        match stream {
            Ok(mut stream) => {
                println!("Connect successfully!");
